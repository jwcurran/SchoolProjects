...
public class ModelEditor extends EditorPart {
	public static final String ID = "com.raytheon.datamodeler.editors.modeleditor";
	...
	public void createPartControl(Composite parent) {
		...
		model = (SeniorModel) selection.getFirstElement();
		if(model == null)
			this.getEditorSite().getPage().closeEditor(this, false);
			
		setPartName("Model Editor");
		...
		deleteButton.addSelectionListener(new SelectionListener(){
			public void widgetSelected(SelectionEvent e) {
				try {
					((TreeView) page.showView(TreeView.ID)).dispose();
				} catch (PartInitException e1) {
					// TODO Auto-generated catch block
					e1.printStackTrace();
				}							
//				((SeniorModel[])treeViewer.getInput())[0] = null;
//    			treeViewer.remove(treeViewer.getInput());
//    			treeViewer.setExpandedState(model,true);
